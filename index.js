...
const actionTimeoutId = setTimeout(() => {
  core.warning(
    `Action timed out after ${MAX_ACTION_DURATION_MS/60000} minutes. This is likely a bug in the action wrapper. Forcing exit.`
  );
  process.exit(1);
...
async function runActionWithWitness(actionDir, witnessOptions) {
  ...
  //
  // FIXED SECTION: Pass environment variables directly without the extra "input-" prefix
  //
  // Instead of only capturing INPUT_INPUT-*, capture all environment vars that start with INPUT_
  // and then strip off the "INPUT_" to form the nested input name
  //
  const nestedInputs = {};
  Object.keys(process.env)
    .filter(key => key.startsWith('INPUT_'))
    .forEach(key => {
      // E.g. if key is 'INPUT_WHO-TO-GREET', inputName becomes 'who-to-greet'
  ...
  Object.keys(nestedInputs).forEach(name => {
    // GitHub Actions looks for INPUT_<inputName in uppercase>
    // So if name is 'who-to-greet', we set envVars.INPUT_WHO-TO-GREET
